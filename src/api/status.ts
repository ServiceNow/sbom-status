...
export async function status(actionArguments: StatusActionArguments) {
  if (actionArguments.bomRecordId == undefined) {
    throw new Error(
      REQUEST_STATUS_ERROR_INSUFFICIENT_DATA("Missing bomRecordId").message,
    );
  }
  ...
  let doWaitForAdditionalInfo =
    actionArguments.fetchPackageInfo || actionArguments.fetchVulnerabilityInfo;
  ...
  while (numPolls++ < MAX_NUM_POLLS) {
    ...
    if (results.result.status === "error") {
      throw new Error(
        `An error occurred: ${JSON.stringify(results.result, null, 2)}`,
      );
    }
  ...
  if (process.env.NODE_ENV !== "test" && processingComplete) {
    ...
    if (
      actionArguments.fetchVulnerabilityInfo &&
    ...
    if (
      actionArguments.fetchPackageInfo &&
    ...
    if (
      actionArguments.fetchLicenseInfo &&
...
export async function _performStatus(
  ...
  return await fetch(statusUrl, {
    ...
    .catch((error) => {
      core.warning(
        `An error occurred while retrieving status of SBOM: ${error.message}`,
      );
      throw error;
    });
}
